...
class HttpRequestTests: XCTestCase {
    func test_HttpRequest_returnsObject_withHttpMethodEnum() {
        let request = HttpRequest(httpMethod: .get, endpoint: "foo", authenticated: false, data: nil)
        XCTAssertEqual(.get, request.httpMethod)
    }

    func test_HttpRequest_returnsObject_withEndpoint() {
        let request = HttpRequest(httpMethod: .get, endpoint: "www.foo.com/api/users/", authenticated: false, data: nil)
